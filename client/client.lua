...
function IsInRestrictedList(groupName)
	for k,v in pairs(Config.RestrictedGroups) do
		if v == groupName then return true end
	end
	return false
end
...
function CustomColour()
	...
	if dialog ~= nil then
		if dialog[1].input == nil then return 0 end
		local number = tonumber(dialog[1].input)
...
function AddNewTag()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		...
		if dialog ~= nil then
			...
			TriggerServerEvent('bixbi_tracker:TaggerAdd', GetPlayerServerId(PlayerId()), true, GetPlayerServerId(closestPlayer), dialog[1].input, tonumber(dialog[2].input) * 60000)
		end
	end
end
